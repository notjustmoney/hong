...
const DetailModal = ({ info }) => {
  ...
  const [btnstate, setBtnState] = useState({ loading: "", disabled: "disabled" });
            ...
            <PopContent>
              ...
              <ContentInfo>
                <PopInfo>
                    ...
                    <Info>
                      <HashTags>
                        {contents.tags.map((tag, index) => (
                          <Link to={`/search?${tag.id}`}><Tag key={index}>#{tag.hashtag}</Tag></Link>
                        ))}
                      </HashTags>
                    </Info>
                  </PopContents>
                  <PopComment id='comments'>
                    {contents.comments.length === 0 && <NoComments>댓글이 없습니다.</NoComments>}
                    {contents.comments.map((comments, index) => (
                        <CommentBox key={index}>
                          ...
                          />
                          <Nickname>
                              <CommentWriter>{comments.writer.profile.nickname}</CommentWriter>
                  ...
                  {loginInfo ? (
                    <>
                      <CommentInput placeholder="댓글 작성" value={comments} onChange={handleComment} />
                      <CommnetSubmit onClick={handleCommentSubmit} content="게시" color="orange" className={[btnstate.loading, btnstate.disabled]}/>
                    </>
                  ) : (
                    <NeedLogin>
